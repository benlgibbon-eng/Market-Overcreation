...
### Quick play mode (no dependencies, no network)

If you just want to see output and tweak windows immediately, run the
dependency-free demo:

```bash
python scripts/rebound_demo.py --trough-window 3 --rebound-window 8
```

This uses synthetic 2024 events so you can "play around" locally before using
live Yahoo Finance pulls.
...
2. Run the analysis script with one or more tickers. The tool queries Yahoo
   ...
   To run a test slice on 2024 events only:

   ```bash
   python scripts/earnings_rebound_analysis.py --tickers PYPL SQ SHOP \
       --limit 20 --start-date 2024-01-01 --end-date 2024-12-31 \
       --output-csv per_event_2024.csv --summary-csv summary_2024.csv
   ```
