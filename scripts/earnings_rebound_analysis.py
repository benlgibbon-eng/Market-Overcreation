...
try:
    ...
    raise SystemExit(
        "Missing dependency `pandas`. Install requirements.txt before running "
        "this script."
    ) from exc
...
def filter_events_by_date(
    ...
    """Restrict earnings events to an inclusive date range."""
    if events.empty:
        return events
    filtered = events
    if start_date is not None:
        filtered = filtered[filtered["earnings_date"] >= start_date]
    if end_date is not None:
        filtered = filtered[filtered["earnings_date"] <= end_date]
    return filtered
...
def analyze_event(
    ...
    # Download a wider calendar window so we reliably capture the requested
    # number of post-event trading days.
    post_event_calendar_days = max(rebound_window * 3, rebound_window + 10)
    window_end = event_date + pd.Timedelta(days=post_event_calendar_days)

    closes = download_price_window(ticker, window_start, window_end)
    closes = closes.sort_index()
    event_day = event_date.normalize()

    pre_event_prices = closes[closes.index < event_day]
    if pre_event_prices.empty:
        return None
    pre_close = float(pre_event_prices.iloc[-1])

    event_day_prices = closes.loc[closes.index == event_day]
    event_close = float(event_day_prices.iloc[-1]) if not event_day_prices.empty else None

    post_event_prices = closes[closes.index > event_day]
    trough_prices = post_event_prices.iloc[:trough_window]
    if trough_prices.empty:
        return None
    trough_idx = trough_prices.idxmin()
    trough_price = float(trough_prices.loc[trough_idx])

    rebound_start = post_event_prices.index.get_indexer([trough_idx])[0]
    rebound_prices = post_event_prices.iloc[rebound_start : rebound_start + rebound_window]
    ...
    days_to_trough = int((trough_idx - event_day).days)
...
def analyze_ticker(
    ...
    events = filter_events_by_date(events, start_date=start_date, end_date=end_date)
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional inclusive lower bound for earnings date (YYYY-MM-DD).",
    )
    parser.add_argument(
        "--end-date",
        type=str,
        default=None,
        help="Optional inclusive upper bound for earnings date (YYYY-MM-DD).",
    )
    ...
    try:
        ...
        raise SystemExit(f"Invalid date input: {exc}") from exc
